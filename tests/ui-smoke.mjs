...
// External connections are live data layers, not display-only rows: one can be
// selected, it then owns the top chrome and the checked radio, and it loses its
// delete action while active. Serving that for real needs a Postgres server, so
// the browser flow runs against responses shaped exactly like the catalogue
// server/db.ts returns for an active external connection
// (tests/store-postgres.mjs covers the server side against a live database).
const externalConnection = {
  id: "ui-smoke-mock",
  name: "Mock warehouse",
  kind: "supabase",
  ssl: true,
  target: "env:UI_SMOKE_PG_URL",
  secretSource: "env",
...
const externalDatabaseId = `external:${externalConnection.id}`;
const localCatalogue = await (await fetch(`${env.VITE_CLAW_TASK_HUB_API_BASE}/databases`)).json();
const externalCatalogue = {
  active: {
    id: externalDatabaseId,
    name: externalConnection.name,
    fileName: externalConnection.name,
    path: externalConnection.target,
    active: true,
    engine: "postgres",
  },
  databases: localCatalogue.databases.map((database) => ({ ...database, active: false })),
};
let externalSelected = false;
await page.route((url) => url.pathname === "/api/db-connections", (route) => (
  route.request().method() === "GET"
    ? route.fulfill({ json: { connections: [externalConnection] } })
    : route.continue()
));
await page.route((url) => decodeURIComponent(url.pathname) === `/api/databases/${externalDatabaseId}/activate`, (route) => {
  externalSelected = true;
  return route.fulfill({ json: externalCatalogue });
});
// Every data refresh re-reads the catalogue from the snapshot, so the snapshot
// has to agree with the activation response or the chrome would flip back.
await page.route((url) => url.pathname === "/api/snapshot" || url.pathname === "/api/refresh", async (route) => {
  const response = await route.fetch();
  const body = await response.json();
  return route.fulfill({ response, json: externalSelected ? { ...body, databases: externalCatalogue } : body });
});
await page.reload();
await page.waitForFunction(() => document.querySelector(".window-tab span")?.textContent?.trim() === "ui-smoke");

await page.getByRole("button", { name: "Manage databases", exact: true }).click();
const externalRadio = page.getByRole("menuitemradio", { name: /^Mock warehouse \(supabase, ok\)$/ });
await externalRadio.waitFor({ state: "visible" });
assert(await externalRadio.isEnabled(), "An external connection is not selectable in the database menu");
assert(await externalRadio.getAttribute("aria-checked") === "false", "An inactive external connection is marked as checked");
assert(await page.getByRole("menuitem", { name: "Delete Mock warehouse", exact: true }).count() === 1, "An inactive external connection lost its delete action");
const activateExternalRequest = page.waitForResponse((response) => (
  decodeURIComponent(new URL(response.url()).pathname) === `/api/databases/${externalDatabaseId}/activate` && response.request().method() === "POST"
));
await externalRadio.click();
await activateExternalRequest;
await page.waitForFunction(() => document.querySelector(".window-tab span")?.textContent?.trim() === "Mock warehouse");

await page.getByRole("button", { name: "Manage databases", exact: true }).click();
const activeExternalRadio = page.getByRole("menuitemradio", { name: /^Mock warehouse \(supabase, ok\) \(active\)$/ });
await activeExternalRadio.waitFor({ state: "visible" });
assert(await activeExternalRadio.getAttribute("aria-checked") === "true", "The active external connection is not checked");
assert(await activeExternalRadio.isDisabled(), "The active external connection can be selected again");
assert(await page.getByRole("menuitem", { name: "Delete Mock warehouse", exact: true }).count() === 0, "The active external connection still offers deletion");
const localRadio = page.getByRole("menuitemradio", { name: "ui-smoke.sqlite", exact: true });
assert(await localRadio.isEnabled(), "The local database cannot be selected while an external connection is active");
assert(await localRadio.getAttribute("aria-checked") === "false", "A local database is still checked while an external connection is active");

externalSelected = false;
const reactivateLocalRequest = page.waitForResponse((response) => (
  new URL(response.url()).pathname.endsWith("/databases/ui-smoke.sqlite/activate") && response.request().method() === "POST" && response.ok()
));
await localRadio.click();
await reactivateLocalRequest;
await page.waitForFunction(() => document.querySelector(".window-tab span")?.textContent?.trim() === "ui-smoke");
